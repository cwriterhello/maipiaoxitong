# 技术精华-Gateway集成Hystrix遇到的巨坑问题

# 问题
Hystrix作为经典的熔断保护机制，虽然现在热度不如Sentinel，但依旧有很多的公司还在使用中，如果是使用Gateway集成Hystrix会有各种各样坑，本文是将踩坑经历介绍出来，如果小伙伴有Gateway集成Hystrix的需求和排查问题的需求，可以好好阅读此文档
...
将线程池线程数设置为300，进行压测，发现并发数在10左右就会进入`fallexecute`熔断。
...
注意`builder.getExecutionIsolationStrategy()`的值为`SEMAPHORE`(上面分析过了)，default_executionIsolationStrategy为`THREAD`。
...
> 更新: 2024-05-16 10:29:31  
> 原文: <https://www.yuque.com/u22210564/ykdrdh/ts7awxmaugyefhrg>